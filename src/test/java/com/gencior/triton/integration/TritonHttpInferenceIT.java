...
/**
 * HTTP integration tests for synchronous and asynchronous inference.
 * Mirrors TritonInferenceIT (gRPC) for full parity.
 */
class TritonHttpInferenceIT extends AbstractTritonHttpIntegrationTest {
    ...
    // ==================== InferResult Accessors (HTTP path) ====================

    @Test
    void inferResult_getOutput_shouldReturnNullInHttpMode() {
        InferInput input = new InferInput("INPUT0", new long[]{2}, TritonDataType.FP32);
        input.setData(new float[]{1.0f, 2.0f});

        InferResult result = client.infer("identity_fp32", List.of(input));

        // HTTP path returns null for protobuf-specific accessors
        assertNull(result.getOutput("OUTPUT0"));
